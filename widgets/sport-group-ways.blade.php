...
购买类别：
<select id="J-select-2" style="display:" name="type">

    <option value=""  >所有类别</option>
    @foreach($sportType as $key => $typename)
        <option value="{{$key}}"  <?php  if($type==$key) echo 'selected';?>>{{$typename}}</option>
    @endforeach
</select>
                <!--
...
// include(realpath(app_path() . '/../widgets/data') . '/province_city.json');
         ...
         * }
            ...
            if (selectedSecond) {
                ...
                thirdSelect = new bomao.Select({realDom:'#J-select-3',cls:'w-3 select-method-2 ', valueKey: 'id', textKey: 'name'});
            }
