...
<dl class="row row-header">
    ...
    @if (in_array($oFilterMethodGroupKey,['hunhe','win-handicapWin','single']))
    ...
    <dd class="col-bet">
        ...
        <div class="line clearfix">
            <span class="text cell">主胜</span>
            <span class="text cell">平</span>
            <span class="text cell cell-last">主负</span>
            <span class="text cell cell-more">更多</span>
        </div>
    </dd>
...
@foreach ($datas as $date => $list)
    ...
    <dl class="row row-date clearfix">
        ...
        ?>
        {{ $date }}　{{ getWeekDay($date) }}12:00 -- {{ getWeekDay(date('Y-m-d',strtotime($date) + 86400)) }}12:00　{{ $iCountSelling }} 场比赛可投注
        <a class="ct" href="#" data-date="{{ $date }}">收起</a>
    </dl>
    @foreach ($list as $data)
    <?php
    $index = 0;
    if (!isset($first_match_id)){
        $first_match_id = null;
        if ($data->is_selling){
            $first_match_id = $data->match_id;
        }
    }
    ?>


    @if($sTabKey == 'correctScore')
        <div class="r row-list-{{$data->match_id}} row-list-league-{{$data->league_id}} row-list-date-{{$data->bet_date}} @if(!$data->is_selling) isStop @if(!isset($dBetDate)) current-date-isStop @endif @endif  @if($data->match_id == $first_match_id)row-list-active@endif">
    @else
    ...
    <dl class="row row-list">
        ...
        <dd class="col-bet">
            @if($sTabKey == 'correctScore')
                @if(!empty($data->score))
                     <span>彩果：<span class="result-item">{{$data->score}}</span></span>
                 @elseif($data->is_cancelled)   
                     <span>彩果：<span class="result-item">取消</span></span>
                 @elseif(!$data->is_selling)
                    <span>彩果：等待结果</span>
                @elseif($data->match_id == $first_match_id)
                     <span data-matchid="{{$data->match_id}}" class="item item-more" data-desc="展开" data-asc="收起">收起 <i class="fa fa-sort-asc"></i></span>
                @else
                    <span data-matchid="{{$data->match_id}}" class="item item-more" data-desc="展开" data-asc="收起">展开<i class="fa fa-sort-desc"></i></span>
                @endif
            @else
                <span data-matchid="{{$data->match_id}}" class="item item-more" data-desc="展开" data-asc="收起">展开<i class="fa fa-sort-desc"></i></span>
            @endif
        ...
        @foreach ($data->method as $method)
            @if($method->identifier != 'win' && $method->identifier != 'handicapWin' && ($oFilterMethodGroupKey == 'hunhe' || $oFilterMethodGroupKey == $method->identifier))
                ...
                @endif
                    <span class="title">
                        {{$method->name}}
                        @if($method->is_single && $sTabKey != 'single')
                            <span class="flag-issingle"></span>
                            <span class="flag-issingle-text">单</span>
                        @endif
                    </span>
