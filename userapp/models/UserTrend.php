...
class UserTrend extends BaseModel
{
    ...
    protected static $aIndexs     = ['5' => 7, '4' => 6, '3' => 5, '3f' => 5, '3m'=>5, '3e' => 5, '2f' => 5, '2e' => 5,'2gy'=>4,'5f'=>7,'5e'=>7];
    // 不同统计类型实际需要遍历的标准列数(球个数 + 分布 + [二星的跨度]) * 10 + [1 (二星的对子)]
    protected static $aCellNums   = ['5' => 60, '4' => 50, '3' => 40, '3f' => 40, '3m'=>40, '3e' => 40, '2f' => 41, '2e' => 41,'2gy'=>30,'5f'=>50,'5e'=>50];
    // 不同统计类型号码截取的起始下标
    protected static $aPositions  = ['5' => 0, '4' => 1, '3' => 0, '3f' => 0, '3m'=>1, '3e' => 2, '2f' => 0, '2e' => 3,'2gy'=>0,'5f'=>0,'5e'=>5];
    ...
    public function getProbabilityOfOccurrenceByParams($iLotteryId, $iNumType = 5, $iBeginTime = null, $iEndTime = null, $iCount = null)
    {
        ...
        $this->iCount = count(explode(',', Series::find($iSeriesId)->valid_nums));
 ...
 */
    public function generatePK10TrendData(& $data, & $statistics,&$aOmissionBarStatus,$iNumType){
        ...
        foreach ($data as $key1 => $oIssue) {
            ...
            if(empty($lastWinNumber)){
                $lastIssue = Issue::where('lottery_id',53)->where('issue','<',$data[$key1][0])->get(['issue','wn_number'])->first();
                if($lastIssue) {
                    $lastWinNumber = $lastIssue->wn_number;
            ...
            $data[$key1][] = $this->countNumberDistribution($aBalls, $iBallsLen);//号码分布

            switch ($this->iBallNum) {
                case '2'://[期号, winball, 第一位,第二位,号码分布,跨度走势,冠亚和大小,冠亚和单双,冠亚和升平降,和值]
                    $data[$key1][] = $this->countNumberRangeTrendPattern($aBalls);
                    $gysum = array_sum($aBalls);

                    $data[$key1][] = $gysum>11?1:0;
                    $data[$key1][] = ($gysum%2)?3:2;
                    $last_gysum = array_sum($lastWinNumber);
                    $data[$key1][] = ($gysum == $last_gysum)?2:($gysum>$last_gysum?1:3);//1升,2平,3降

                    $data[$key1][] = $gysum;
                    $this->countPairAndRangeOmissionPk10($data, $key1, $tempOmissionForPair, $tempOmissionForRange, $aTimes, $aAvgOmission, $aMaxOmission, $aMaxContinous, $aMaxContinousCache);
    ...
    public function countPairAndRangeOmissionPk10(& $data, $i, & $tempOmissionForPair, & $tempOmissionForRange, & $aTimes, & $aAvgOmission, & $aMaxOmission, & $aMaxContinous, & $aMaxContinousCache ){
    ...
    public function generateTrendData(& $data, & $statistics, & $hotAndCold, & $aOmissionBarStatus, $iNumType)
    {
        ...
        foreach ($data as $key1 => $oIssue) {
            ...
            //     exit;
            // }
//pr($aOmissionBarStatus);pr($key1);pr($aBalls);
            switch ($this->iBallNum) {
                case '2':
...
//        exit;
    ...
    public function generateK3TrendData(& $data, & $statistics, & $hotAndCold, & $aOmissionBarStatus, $iNumType)
    {
        ...
        // for ($key1 = $this->iDataLen - 1; $key1 >= 0; $key1--) {
        //     $oIssue = $data[$key1];
        $threeSame = $twoNoSame = $threeNoSame = $threeLinkSame = 0;
        ...
        foreach ($data as $key1 => $oIssue) {
            ...
            foreach ($aBalls as $key2 => $value) {
                $value = intval($value);
                $arr = $this->makeK3RowData($key2, $key1, $value, $aOmissionBarStatus, $aLostTimes, $aTimes, $aAvgOmission, $aMaxOmission, $aMaxContinous, $aMaxContinousCache);
                $data[$key1][$key2 + 2] = $arr;
                // $aAllNumbers[$key2 + 2] = $value;
            }
        ...
        foreach($data1 as $key1 => $val1){
            ...
            foreach($val1 as $key2 => $val2){
                ...
                if($val2 == 0){
                    ...
                    if($curMaxContinous > $countMaxContinous) $countMaxContinous = $curMaxContinous;
                    $curMaxContinous = 0;
                }
            }
    ...
    private function makeRowData($iNum, $key1, $sBall, & $aOmissionBarStatus, & $aLostTimes, & $aTimes, & $aAvgOmission, & $aMaxOmission, & $aMaxContinous, & $aMaxContinousCache){
        ...
        for($i = 0; $i < $iCount; $i++){
//            $iNumber = $this->iType == 1 ? $i : $i + 1;
            $iNumber = ($this->iCount == 10 && $this->iType!=19) ? $i : $i + 1;
     ...
     *       ]
     */
    private function makeK3RowData($iNum, $key1, $sBall, & $aOmissionBarStatus, & $aLostTimes, & $aTimes, & $aAvgOmission, & $aMaxOmission, & $aMaxContinous, & $aMaxContinousCache){
        $result = [];
        //$iCount = $this->iType == 1 ? 10 : 11;
        $iCount = $this->iCount;
        // 11选5的遗漏次数数组排序是万千百十个位[0-10][11-21][22-32][33-43][44-54]
...
//            $iNumber = $this->iType == 1 ? $i : $i + 1;
    ...
    private function generateK3Ddata($aBalls, & $data, $key1){
        ...
        //三同号
        if(min($aBalls) == max($aBalls)) $threeSame = 0;
        else $threeSame = empty($preData) ? 1 : $preData[count($data[$key1])][0] + 1;
        $data[$key1][] = [$threeSame];


        //二同号
        if(count(array_unique($aBalls)) < count($aBalls)) $twoNoSame = 0;
        else $twoNoSame = empty($preData) ? 1 : $preData[count($data[$key1])][0] + 1;
        $data[$key1][] = [$twoNoSame];

        //三不同
        if(count(array_unique($aBalls)) == count($aBalls)) $threeNoSame = 0;
        else $threeNoSame = empty($preData) ? 1 : $preData[count($data[$key1])][0] + 1;
        $data[$key1][] = [$threeNoSame];

        //三连号
        if((count(array_unique($aBalls)) == count($aBalls) && (max($aBalls) - min($aBalls)) == 2)) $threeLinkSame = 0;
        else $threeLinkSame = empty($preData) ? 1 : $preData[count($data[$key1])][0] + 1;
...
//        $iCount = $this->iCount == 10 ? 10 : 12;
        $iCount = $this->iCount;
        $iStart = ($this->iCount == 10 && $this->iType!=19) ? 0 : 1;
        if($iStart === 0) $iCount--;
    ...
    private function countDistributionOmission( & $data, $i, & $tempOmissionForDistribution, & $aTimes, & $aAvgOmission, & $aMaxOmission, & $aMaxContinous, & $aMaxContinousCache )
    {
        $iIndex = $this->iIndex;
        $iCount = $this->iCount;
        $iDistributionStart = $this->iBallNum *$this->iCount  + intval($this->iBallNum == 2);
