...
class ClearBeanstalkdQueueCommand extends Command {
	...
	public function fire()
	{
		...
		$aQueueCfgs = Config::get('queue.connections');
//        array_unshift($queues, $queue);

        foreach ($aQueueCfgs as $aQueueCfg){
            if ($aQueueCfg[ 'driver' ] != 'beanstalkd'){
                continue;
            }
            $sQueue = $aQueueCfg[ 'queue' ];
            $this->info(sprintf('Clearing queue: %s',$sQueue));

            $pheanstalk->useTube($sQueue);
            $pheanstalk->watch($sQueue);
