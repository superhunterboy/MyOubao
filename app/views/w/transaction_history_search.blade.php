        ...
        <td style="width:980px">
            <table>
                ...
                <tr>
                    ...
                    <td style="width:180px"><input class="form-control input-sm" type="text" name="serial_number" value="{{@$aSearchFields['serial_number']}}"></td>
                    <td style="width:80px" class="text-right">{{__('_transaction.ip')}}：</td>
                    <td style="width:260px"><input class="form-control input-sm" type="text" name="ip" value="{{@$aSearchFields['ip']}}"></td>
                    <td style="width:80px" class="text-right">{{__('_transaction.admin_user_id')}}：</td>
                    <td style="width:180px"><select name="admin_user_id" style="width:100%" class="form-control select-sm"><option value>{{__('_transaction.all')}}</option>@foreach($aAdminUsers as $id => $name)<option value="{{$id}}" @if(@$aSearchFields['admin_user_id']==$id)selected='selected' @endif>{{$name}}</option>@endforeach</select></td>
                </tr>
                <tr>
                    ...
                    <td >
                        <input class="form-control input-sm" type="text" style="width:80px;" name="amount[]" value="{{@$aSearchFields['amount'][0]}}"> {{__('_transaction.to')}}
                        <input class="form-control input-sm" type="text" style="width:80px;" name="amount[]" value="{{@$aSearchFields['amount'][1]}}">
                    </td>
                </tr>
                <tr>
...
<script type="text/javascript">
    $(function () {
        ...
        $('#download_transaction').click(function () {
            $('#transaction_search_form').attr('action','/transaction-histories/download');
            $('#transaction_search_form').submit();
        });

        $('#submit_transaction').click(function () {
            $('#transaction_search_form').attr('action','/transaction-histories');
            $('#transaction_search_form').submit();
        });
