...
<table style="width:100%"><tr>
        <td style="width:840px">
            <table>
                ...
                <tr>
                    <td style="width:80px" class="text-right">合买编号：</td>
                    <td  style="width:180px">
                ...
                {{--<tr>--}}
                    ...
                    {{--<td  style="width:180px">--}}
                        ...
                        {{--foreach ($validBuyType as $key => $value) {--}}
                            {{--$validBuyType[$key] = __('_manjcproject.' . strtolower(\Str::slug($value)));--}}
                        {{--}--}}
                        {{--?>--}}
                ...
                <tr>
                    ...
                    <td  style="width:180px">
                        ...
                        foreach ($validStatus as $key => $value) {
                            $validStatus[$key] = __('_manjcgroupbuy.' . strtolower($value));
                        }
                        ?>
                    ...
                    <td  style="width:180px">
                        ...
                        foreach ($validPrizeStatus as $key => $value) {
                            $validPrizeStatus[$key] = __('_manjcgroupbuy.' . strtolower($value));
                        }
                        ?>
                    ...
                    <td style="width:80px" class="text-right">显示状态：</td>
                    <td  style="width:180px">
                        <?php
                        foreach ($validTypes as $key => $value) {
                            $validTypes[$key] = __('_manjcgroupbuy.' . strtolower($value));
                        }
                        ?>
                        <select name="show_type" style="width:100%" class="form-control select-sm">
                            <option value>不限</option>
                            @foreach ($validTypes as $key => $value)
                                <option value="{{ $key }}" {{ @$aSearchFields['show_type'] === (string)$key ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
...
//            $('#download_flag').val('download');
            $('#project_search_form').attr('action', '/jc-group-buys/download');
//            $('#_action').val('/projects/download');
            $('#project_search_form').submit();
        });
        $('#submitForm').click(function (event) {
            $('#project_search_form').attr('action', '/jc-group-buys');
            $('#project_search_form').submit();
        });
        //时间控件
