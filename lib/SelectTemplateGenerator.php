...
class SelectTemplateGenerator {
    ...
    private function _generate3LevelData($aWidget)
    {
        ...
        // $aConditions    = isset($aWidget['condition']) ? $aWidget['condition'] : null;

        // if ($aConditions) {
        //     $aMainData = $sMainModel::where($aConditions['column'], $aConditions['operation'], $aConditions['value'])->get($aColumns1);
        //     $queries = DB::getQueryLog();
        //     $last_query = end($queries);
        //     @file_put_contents('/home/snowan/tmp/generate.log', $last_query, FILE_APPEND);
        // } else {
        ...
        foreach ($aMainData as $oMainData) {
            ...
            if ($sChildModel) {
                ...
                $oQuery = $sChildModel::where($sParentKey1, '=', $oMainData->id)->whereNull($sParentKey2);
                // if ($aConditions) {
                //     $oQuery = $oQuery->where($aConditions['column'], $aConditions['operation'], $aConditions['value']);
                // }
                $aChildData = $oQuery->orderBy('sequence')->get($aColumns2);
    ...
    private function _generateData($aWidget)
    {
        ...
        $oQuery       = ($iLevel == self::LEVEL_1 || $sMainModel != $sChildModel) ? $sMainModel::whereRaw('1=1') : $sMainModel::whereNull($sParentKey);

        $aConditions    = isset($aWidget['condition']) ? $aWidget['condition'] : null;

        if ($aConditions) {
            $aMainData = $oQuery->where($aConditions['column'], $aConditions['operation'], $aConditions['value'])->get($aColumns);
            // $queries = DB::getQueryLog();
            // $last_query = end($queries);
            // @file_put_contents('/home/snowan/tmp/generate.log', $last_query, FILE_APPEND);
        } else {
    ...
    /**
     * 获取按照指定配置格式的json数据
     * @param type $aWidget     组件配置信息
     * @return type     json格式的数据
     */
    public function generateJsonData($aWidget) {
        $iLevel = $aWidget['level'];
        $sOutput = $aWidget['output'];
        $sName = isset($aWidget['name']) ? $aWidget['name'] : 'name';
        $sDataName = isset($aWidget['dataName']) ? $aWidget['dataName'] : 'selectorData';
