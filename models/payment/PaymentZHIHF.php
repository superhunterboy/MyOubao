 ...
 *
 */
class PaymentZHIHF extends BasePlatform {
    ...
    public $unSignColumns = [ 'sign_type', '_url'];
...
//            pr($aInputData);
        ...
        $sQueryString = '';
        $sForSafe = '';
        $sSign = $aInputData['sign'];
        unset($aInputData['sign']);
        foreach ($aInputData as $key => $value) {
            if ($aNeedKeys) {
                if (!in_array($key, $aNeedKeys)) {
                    continue;
        ...
        $sQueryString = implode('&', $aInputValues);
        $dinpaySign = base64_decode($sSign);
        $dinpay_public_key = openssl_get_publickey($this->pubKey);
        $flag = openssl_verify($sQueryString, $dinpaySign, $dinpay_public_key, OPENSSL_ALGO_MD5);
        return $flag;
    }
