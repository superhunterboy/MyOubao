...
(function(host, GameMethod, undefined){
		...
		miniTrend_createRowHtml:function(){
			...
			$.each(data, function(i){
				...
				html.push('<tr class="'+ trcls +'">');
					...
					html.push('</span></td>');
					xtText = '-';
					if((item['balls'][2] - item['balls'][1] == 1) && (item['balls'][1] - item['balls'][0] == 1)){
						xtText = '<span class="color-hl">三连号</span>';
					}
	...
	var html_head = [];
		...
		html_head.push('<ul class="ball-section ball-section-k3-sanlianhao">');
	var numberHash = {
		'0-0':'123', '0-1':'234',  '0-2':'345',  '0-3':'456',
	};
		//每行
	var html_row = [];
		html_row.push('<li>');
		html_row.push('<ul class="ball-content">');
			$.each(['<#=row#>-0','<#=row#>-1','<#=row#>-2','<#=row#>-3'], function(i){
