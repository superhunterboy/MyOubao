...
(function(host, GameMethod, undefined){
	var defConfig = {
		name:'renxuan.renxuan2.zuxuanfushi',
		//玩法提示
		tips:'',
		//选号实例
		exampleTip: ''
	...
	var pros = {
		...
		getLottery:function(isGetNum){
			...
			optionArr = me.combine(optionIndex, 2);

			$.each(optionArr, function(i){
				optionArr[i] = optionArr[i].join(',');
			});

			resultLast = me.combination([optionArr, result]);

			//console.log(resultLast);
			
			me.isBallsComplete = resultLast.length > 0 ? true : false;
			return resultLast;
