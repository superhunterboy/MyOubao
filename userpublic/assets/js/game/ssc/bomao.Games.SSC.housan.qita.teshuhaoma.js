...
(function(host, GameMethod, undefined) {
		...
		miniTrend_createRowHtml:function(){
			...
			$.each(data, function(i){
				...
				html.push('<tr class="'+ trcls +'">');
					...
					//判断顺子，顺子不区分位置
					shunArr = [];
					shunArr.push(item['balls'][2]);
					shunArr.push(item['balls'][3]);
					shunArr.push(item['balls'][4]);
					shunArr.sort();

					if(item['balls'][2] == item['balls'][3] && item['balls'][2] == item['balls'][4]){
						xtText = '豹子';
					}else if(item['balls'][2] == item['balls'][3] || item['balls'][2] == item['balls'][4] || item['balls'][3] == item['balls'][4]){
						xtText = '对子';
					}else if((shunArr[2] - shunArr[1] == 1) && (shunArr[1] - shunArr[0] == 1)){
						xtText = '顺子';
					}else{
						xtText = '-';
					}
	...
	html_row.push('<ul class="ball-content ball-content-long" style="padding-left:140px;">');
