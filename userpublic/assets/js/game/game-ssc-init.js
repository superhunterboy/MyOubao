(function(host, name, Event, undefined){
	...
	var pros = {
		...
		methodBallMaxMultiple:99999999,
		//id : methodid
		//unit : money unit (1 | 0.1)
		getLimitByMethodId:function(id, unit){
			var unit = unit || 1,
				maxnum = Number(this.getMethodById(id)['max_multiple']);
			return maxnum / unit;
		},
		//注单提交地址
...
(function(){
	...
	var init = function(config){
		...
		var Games = bomao.Games;
			//游戏实例
			bomao.Games.SSC.getInstance({'jsNameSpace': 'bomao.Games.SSC.'});
			//游戏玩法切换
			...
			//消息类
			bomao.Games.SSC.Message.getInstance();
			...
			var updateView = function(){
                ...
                $('#J-lottery-balls-lasttime').html('<em>' + lastBalls.join('</em><em>') + '</em>');
			...
			Games.getCurrentGame().addEvent('afterSwitchGameMethod', function(e, id) {
				...
				if(method.getName().indexOf('longhu.') == 0){
					//龙虎
					prizes.push((2 / 0.45 * userPrize / 2000 * unit).toFixed(2));
					//和
					prizes.push((2 / 0.1 * userPrize / 2000 * unit).toFixed(2));
					$('#J-method-prize').html(prizes.join(' - '));
				}else if(methodCfg['is_enable_extra'] == 1 && method.getName().indexOf('hezhi.wuxing.hezhi') == 0){
					var prizesArr = [],
						p;
					...
					prizes.push((prizesArr[0] * unit).toFixed(2));
					prizes.push((prizesArr[prizesArr.length - 1] * unit).toFixed(2));
					$('#J-method-prize').html(prizes.join(' - '));
				}else{
					//更新玩法单注奖金
			...
			Games.getCurrentGame().addEvent('afterSelectBall', function(e, param){
				var method = Games.getCurrentGame().getCurrentGameMethod(),
					...
					prizes = [],
					unit = Number(Games.getCurrentGameStatistics().getMoneyUnitDom().getValue());
				if(methodCfg['is_enable_extra'] == 1 && method.getName().indexOf('hezhi.wuxing.hezhi') == 0){
					...
					setTimeout(function(){
						...
						if(prizesArr.length == 0){
							prizes.push((allprizesArr[0] * unit).toFixed(2));
							prizes.push((allprizesArr[allprizesArr.length - 1] * unit).toFixed(2));
							$('#J-method-prize').html(prizes.join(' - '));
							return;
						}
						if(prizesArr.length == 1){
							prizes.push(prizesArr.length > 0 ? (prizesArr[0] * unit).toFixed(2) : 0);
							$('#J-method-prize').html(prizes.join(' - '));
						}else{
							prizes.push(prizesArr.length > 0 ? (prizesArr[0] * unit).toFixed(2) : 0);
							prizes.push(prizesArr.length > 0 ? (prizesArr[prizesArr.length - 1] * unit).toFixed(2) : 0);
							$('#J-method-prize').html(prizes.join('-'));
						}
