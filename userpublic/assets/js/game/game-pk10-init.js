(function(host, name, Event, undefined){
    var gameConfigData = global_game_config_ssc,
        defConfig = {
        },
        nodeCache = {},
        methodCache = {},
    ...
    gameConfigData['jsPath'] = gameConfigData['jsPath'].replace('ssc', 'pk10');
...
(function(){
    ...
    var init = function(config){
        ...
        //游戏实例
        bomao.Games.PK10.getInstance({'jsNameSpace': 'bomao.Games.PK10.'});
        //游戏玩法切换
        ...
        //消息类
        bomao.Games.PK10.Message.getInstance();
        ...
        var updateView = function(){
            ...
            $('#J-lottery-balls-lasttime').html('<em>' + lastBalls.split(',').join('</em><em>') + '</em>');
            $('#J-lottery-balls-lasttime > em').each(function(){
                $(this).addClass('bnum-' + $.trim($(this).text()));
            });
            ...
            if(pageLastBalls != lastBalls){
                //翻转动画
                flipBalls(lastBalls.split(","));
                pageHiddenInput.value = lastBalls;
            }
        ...
        Games.getCurrentGameTypes().addEvent('beforeChange', function(e, id){
            ...
            //hezhi xia dandu fandianss
            (function () {
                var r_dom = $('#J-balls-statistics-panel .w-2'),
                    cfg = Games.getCurrentGame().getGameConfig().getInstance(),
                    subgroup = Number(cfg.getConfig('subtract_prize_group')),
                    min = Number(cfg.getConfig('bet_min_prize_group')),
                ...
                if(id==269){
                    var r_min = Number(min-30),
                        r_max = Number(umax-30);

                            r_dom.find('.info input').attr({
                                'data-realvalue':r_max,
                                value:r_max +' - '+ ls_maxper+'%'
                            }).val(r_max +' - '+ ls_maxper+'%');
                            r_dom.find('.choose-list-cont a:eq(0)').attr('data-value',r_min +' - ' + ls_minper+'%').text(r_min +' - ' + ls_minper+'%');
                            r_dom.find('.choose-list-cont a:eq(1)').attr('data-value',r_max +' - ' + ls_maxper+'%').text(r_max +' - '+ ls_maxper+'%');
                            // $('#J-select-rebate option:eq(0)').attr('value',r_min).text(r_min +' - '+ ls_minper+'%');
                            // $('#J-select-rebate option:eq(1)').attr('value',r_max).text(r_max +' - '+ ls_maxper+'%');
                }else {
                    ...
                    r_dom.find('.info input').attr({
                        'data-realvalue': umax,
                        value: umax + ' - '+ ls_maxper+'%'
                    }).val(umax + ' - '+ ls_maxper+'%');
                    r_dom.find('.choose-list-cont a:eq(0)').attr('data-value', min).text(min +' - '+ ls_minper+'%');
                    r_dom.find('.choose-list-cont a:eq(1)').attr('data-value', umax).text(umax + ' - '+ ls_maxper+'%');
        ...
        Games.getCurrentGame().addEvent('afterSwitchGameMethod', function(e, id) {
            ...
            //$('#J-list-historys').show().css('marginTop', $('#J-gametypes-panel-cont').height() * -1 + 45);

            //更新玩法单注奖金
            /**
             prize = Number(methodCfg['prize']) * unit;
             prize = prize.toFixed(4);
             //prize = bomao.util.formatMoney(prize, 4);
             prize = ('' + prize).split('.');
             prize[1] = '<i>' + prize[1] + '</i>';
             prize = prize.join('.');
             $('#J-method-prize').html(prize);
             **/
            if(methodCfg['is_enable_extra'] == 1 && (method.getName().indexOf('zhixuanhezhi.hezhi.guanyahezhi') == 0 || method.getName().indexOf('liangmianpan.zhixuan.guanyahezhi') == 0)){
                var prizesArr = [],
                    p;
                for(p in methodCfg['extra_prize']){
                    if(methodCfg['extra_prize'].hasOwnProperty(p)){
                        prizesArr.push(Number(methodCfg['extra_prize'][p]));
                    }
                }
                prizesArr.sort(function(a, b){
                    return a - b;
                });
                prizes.push((prizesArr[0] * unit).toFixed(4));
                prizes.push((prizesArr[prizesArr.length - 1] * unit).toFixed(4));
            ...
            //根据玩法选球区域高度调整小走势图可见条数
            (function(){
                var ballscont = $('#J-balls-main-panel'),
                    height = ballscont.height();
                if(height < 534){
                    height = 534;
                }else{
                    height = (Math.floor(height/83) + 5 )*83 - 43;
                }
                $('#J-minitrend-cont').height(height);
            })();
        });
        ...
        $('#J-gametyes-menu-panel').on('mouseover', '.types-item', function(){
            ...
            var tipdom = TipRulePanel.getDom(),
                x = tipdom.width()/2 * -1 + el.width()/2,
                y = tipdom.height() * -1 - 30,
                offset;
            TipRulePanel.show(x, y, el);


            offset = tipdom.offset();
            if($(window).scrollTop() > offset.top){
                tipdom.css('top', offset.top + tipdom.height() + 70);
                tipdom.removeClass('j-ui-tip-b').addClass('j-ui-tip-t');
            }else{
                tipdom.removeClass('j-ui-tip-t').addClass('j-ui-tip-b');
            }
        ...
        //根据选球内容显示奖金范围和可选倍数
        Games.getCurrentGame().addEvent('afterSelectBall', function(e, param){
            var method = Games.getCurrentGame().getCurrentGameMethod(),
                ...
                prizes = [],
                unit = Number(Games.getCurrentGameStatistics().getMoneyUnitDom().getValue());
            if(methodCfg['is_enable_extra'] == 1 && method.getName().indexOf('zhixuanhezhi.hezhi.guanyahezhi') == 0){
                ...
                setTimeout(function(){
                    ...
                    //限制可选最大倍数
                    maxmultiple = Games.getCurrentGameStatistics().getPrizesMultipleBound()['min'] / unit;
                    if(Games.getCurrentGameStatistics().getMultip() > maxmultiple){
                        Games.getCurrentGameStatistics().setMultipleDom(maxmultiple);
                    }
            ...
            if(methodCfg['is_enable_extra'] == 1 && method.getName().indexOf('liangmianpan.zhixuan.guanyahezhi') == 0){
                for(var p in methodCfg['extra_prize']){
                    if(methodCfg['extra_prize'].hasOwnProperty(p)){
                        allprizesArr.push(Number(methodCfg['extra_prize'][p]));
                    }
                ...
                setTimeout(function(){
                    for(i = 0; i < len; i++){
                        for(j = 0; j < allBallsDoms[i].length; j++){
                            if(allBallsDoms[i][j].className.indexOf('ball-number-current') != -1){
                                multipleArr.push(Number(extracfg['' + Number($(allBallsDoms[i][j]).attr('data-myindex'))]));
                                prizesArr.push(Number(extraprizecfg['' + Number($(allBallsDoms[i][j]).attr('data-myindex'))]));
                            }
                        }
                    }
                    prizesArr.sort(function(a, b){
                    ...
                    if(prizesArr.length == 1){
                        ...
                        var minn = (prizesArr[0] * unit).toFixed(4);
                        var maxn = (prizesArr[prizesArr.length - 1] * unit).toFixed(4);
                        if(minn == maxn){
                            prizes.push(prizesArr.length > 0 ? minn : 0);
                        }else{
                            prizes.push(prizesArr.length > 0 ? minn : 0);
                            prizes.push(prizesArr.length > 0 ? maxn : 0);
                        }
                        $('#J-method-prize').html(prizes.join('-'));
                    }
        ...
        (function(){
            ...
            var formatRowBets = function(row){
                var projectViewBaseUrl = Games.getCurrentGame().getGameConfig().getInstance().getProjectViewBaseUrl();
                var tpl = betsRowTpl,cls = [];
        ...
        (function(){
            ...
            MSG.addEvent('afterSuccess', function(e, data){
                ...
                //data['data']['lotteryBalls'] = '05 04 03 02 01';
                var cfg = Games.getCurrentGame().getGameConfig().getInstance(),
                    lastNumber = '' + cfg.getLastGameNumber(),
